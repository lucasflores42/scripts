...
********************************************************************/
void calculo_investimento(double  *investimento, int state[N])
{
	int i;
	for(i=0;i<N;i++)
	{
		double k = gsl_rng_uniform(rand_vec);

		if(state[i] == 0){investimento[i] = 0.;}
		else{investimento[i] = 1.;}
		//printf(" %d %lf\n",state[i],investimento[i]);
	}
}
/********************************************************************
...
void calculo_mcs(double *payoff, int state[N], int **viz, double r, double gama, double delta, int topologia[N], double *investimento)
{
	...
	{					
		...
		if (state[vizinho] != state[x])
		{
			...
			Px = payoff[state[sitio]] -1*state[sitio];//- investimento[state[sitio]];
		
			calculo_numero(state,vizinho,viz,topologia);
			calculo_payoff(payoff,r,gama,delta,vizinho,topologia,state,investimento,viz);
			Py = payoff[state[vizinho]] -1*state[vizinho];//- investimento[state[vizinho]];
			//printf("%d %lf %lf\n",state[vizinho],payoff[state[vizinho]],payoff[DEFECTOR]);
			...
			{
				sitio2 = viz[sitio][y];
				calculo_numero(state,sitio2,viz,topologia);
...
//------------------------------------------------------------------------------------
	...
	calculo_investimento(investimento,state);
